...
Heuristic::Heuristic(const Options &opts)
    : Evaluator(opts.get_unparsed_config(), true, true, true),
      ...
      preferred_selection_probability(opts.get<double>("preferred_selection_probability")) {
}
...
bool Heuristic::is_preferred(const OperatorProxy &op) {
    return preferred_operators.contains(op.get_ancestor_operator_id(tasks::g_root_task.get()));
}
...
void Heuristic::add_options_to_parser(OptionParser &parser) {
    ...
    parser.add_option<double>(
        "preferred_selection_probability",
        "the probability of selecting a preferred operator",
        "1.0");
}
